...
export default function Chat() {
  ...
  useEffect(() => {
    if (gamePhase === 'HIDING') {
      addChatMessage('Hiding phase started! Props have 30 seconds to conceal themselves.', 'System', 'SYSTEM', false, 'global');
    } else if (gamePhase === 'HUNTING') {
      addChatMessage('Hunting phase started! Hunters are released!', 'System', 'SYSTEM', false, 'global');
    } else if (gamePhase === 'GAME_OVER') {
      addChatMessage('Game Over!', 'System', 'SYSTEM', false, 'global');
    }
  }, [gamePhase]);
  ...
  const onlineCount = Object.keys(otherPlayers).length + 1; // other players + local player
          ...
          <div className="flex items-center justify-between bg-zinc-950/90 border border-zinc-800/80 border-b-0 px-4 py-2 rounded-t-xl backdrop-blur-md">
            ...
            <div className="flex items-center space-x-2">
              <div className="flex items-center space-x-1.5 bg-zinc-900 px-2 py-0.5 rounded-full border border-zinc-800">
                ...
                <span className="text-[10px] font-mono font-bold text-zinc-400">{onlineCount} ONLINE</span>
              </div>
              <button
